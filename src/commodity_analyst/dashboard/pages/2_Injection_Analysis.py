...
if not current.empty:
    ...
    if deficit > 500:
        st.error(f"Injection deficit is **{deficit:.0f} GWh/d**  - a large gap that will be very difficult to close in the {days_left} days remaining before November 1. Strongly supports higher prices.")
    elif deficit > 0:
        st.warning(f"Injection deficit is **{deficit:.0f} GWh/d**  - injection needs to pick up to hit 90%. {days_left} days remaining.")
    elif days_left > 0:
        st.success(f"On track  - current injection pace exceeds the required rate by **{abs(deficit):.0f} GWh/d**. {days_left} days remaining before November 1.")
    else:
...
if not current.empty:
    ...
    if days_left > 0:
        fig1.add_hline(y=required_rate, line_dash="dot", line_color="red", opacity=0.6,
